...
int main(int argc, char *argv[])
{
	...
	unmountbuf = malloc(SHELLCMD_LEN);
	sprintf(unmountbuf, "sudo diskutil unmountDisk %s", argv[2]);
	sprintf(cmdbuf, "sudo dd conv=fsync if=load.imx of=%s bs=512 seek=2", argv[2]);
